...
### Paso 1 (Gist) y Paso 2 (barrido de productos, motor empírico) — 19-sep-2026

**Paso 1:** sin `GITHUB_GIST_TOKEN`/`GIST_ID` en esta sesión. Preparado `dd_wr/audit_mgc_live_mix.py` (solo lectura; el usuario lo corre): mezcla TP/SL/FLATTEN real, WR condicional (convención del Telegram = TP/(TP+SL), la misma de 47.4%), test binomial del FLATTEN share vs T3/2 años. Con n=9 los tests tienen poder casi nulo. PENDIENTE: resultado real del usuario.

**Ventanas horarias por producto (evidencia):** los parquets de M2K/MCL/M6E/ZN/ZC solo cubren 8:30–15:00 CT (ZC 8:30–13:15); NO hay datos fuera de eso (mismo problema que tuvo MGC: filtro de equity index). Perfil de volumen por 30 min: M2K y ZC pico en la primera barra (8:30, apertura real de su sesión → ventana completa); MCL meseta 8:30–10:00 (pit abre 8:00 CT, posible truncamiento); M6E pico interior a 9:00; ZN picos 9:00 y 15:00 (datos de 7:30 CT ausentes). Entrada = primer bar >= apertura+13 min (misma regla ENTRY_WAIT_MINUTES de MGC); flatten = 30 min antes del fin de datos (convención del scheduler MGC, no evidencia independiente). M6E y ZN se corrieron con dos aperturas (8:30 y 9:00). Productos con ventana posiblemente truncada (MCL, M6E, ZN) quedan PROVISIONALES hasta re-fetch (requiere MASSIVE_API_KEY del usuario).
**HALLAZGO LATERAL sobre MGC:** en `mgc_5min_2y_corrected_window` el volumen medio por barra pico es 8:30–9:30 CT (2,267 y 2,105) — NO en 7:00–8:30 (1,215–1,580). La extrapolación monótona hacia atrás del 07-sep-2026 (que motivó 7:13 CT) no se confirma con la ventana corregida; 7:13 CT opera a ~55–65% del pico de volumen. No cambia producción; no verificado si 7:13 tiene otra razón (delay de datos, etc.).

**Barrido:** `dd_wr/product_sweep.py`, 196 configs (6 productos, 8 combinaciones producto/entrada, 4 anchos c×R con R = rango mediano entrada→flatten, 6 RR, MGC + ancho 364), SL$≈$2,184, nc=clip(round(2184/SL$·contrato),1,cap), TP neto>=$150. Motor empírico (bar-walk de sesión + bootstrap del PnL real, nc fijo, sin fix balance>0). Reproduce MGC 364/364: p50 $16,043. Resultados `dd_wr/product_sweep_results.csv`.
**Resultado:** ningún otro producto supera a MGC. Mejor por producto (p50, RR=1.0 en todos): M2K $23.9k (154/154 nc=28), MCL $17.7k (64/64 nc=30), M6E $10.1k–$14.1k, ZN $6.2–6.8k, ZC $4.3k. Dentro de MGC el ancho del SL domina sobre el RR: SL=238/238 nc=9 (0.5R) $29.8k vs baseline 364/364 $16.0k; en dyn-nc real: 0.25R $42.4k, 0.35R $41.3k, 0.4R $38.3k, 0.5R $29.2k, 0.6R $17.9k (RR=1). El máximo NO está en el interior: está en el borde inferior del grid (c=0.25 es el menor probado).
**Robustez (primera pasada, `dd_wr/mgc_width_robustness.py`, NO es el DD completo):** por tercio los anchos angostos ganan en los 3 tercios (T3 $32–38k vs baseline $14.2k); Scaling Plan permite nc>=30 a balance<2000 (nc=18 OK); slippage 1–3 ticks: 0.4R RR1 $38.3k→$36.7k/$34.3k/$30.3k, baseline $15.7k→$15.2k/$14.5k/$13.7k. ALERTAS: (1) no-monotonía entre anchos vecinos (0.5R RR1 $29k vs 0.6R RR1 $18k, salto de nc 9→8) sugiere sensibilidad al camino de 515 días; (2) EV/trade +$100 con SE≈$84 (~1.2σ): no significativo; (3) grid multiple-testing: 196 configs sin corrección aplicada; (4) friccion probada solo como slippage fijo; (5) borde de grid sin explorar; (6) dependencia/Kelly/H1-H2 formal/data snooping pendientes. NO es candidato; producción sin cambios.

### SEGUIMIENTO ACTIVO (no confirmación definitiva): mezcla real de producción de Cerebro 2 vs backtest (19-sep-2026)

Datos reales del Gist (proveídos por el usuario, `dd_wr/audit_mgc_live_mix.py`): 7 entradas resueltas, FLATTEN 4/7 = 57.1%, WR condicional 33.3% (1 TP / 2 SL, n=3). Test binomial exacto (dos colas) del FLATTEN share: vs T3 23.8% p=0.060; vs últimos 120d 27.5% p=0.096; vs promedio 2 años 50.9% p=1.0. El WR condicional descansa en solo 3 trades resueltos TP/SL: p=1.0 contra 47.4% y contra 50% — sin información. La dirección apunta al lado pesimista del rango ($14–16k, y su régimen reciente T3 ≈$14k, más que al optimista), pero con n=7 es una SEÑAL A VIGILAR, no una confirmación. Nota: los datos de backtest terminan el 04-sep-2026; el solape con el período en vivo es parcial. Vigilar hasta el deadline del 1-oct-2026; re-correr el audit con cada ciclo nuevo.

### Origen de la hora de entrada 7:13 CT / ENTRY_WAIT_MINUTES=13 (19-sep-2026, revisión del log original)

- `ENTRY_WAIT_MINUTES=13`: basado ÚNICAMENTE en el delay medido de la API de Massive (`probe_massive_mgc_delay.py`: 08-sep 9.43 min prom., 09-sep 9.50 min prom., máx observado 9.94 min, +~3 min de margen). Es un buffer de latencia de datos, no una decisión de mercado. (log 08-sep y 09-sep-2026)
- Apertura 7:00 CT (RTH_OPEN 7:00): basada en (a) CME/COMEX sesión heredada del pit 7:20–12:30 CT, (b) fuentes externas del overlap Londres-NY 7:00–11:00 CT, y (c) una extrapolación interna: "el volumen decae monótonamente desde la primera barra (8:30), por lo tanto 7:00–8:30 es probablemente MÁS activo". El log dice literalmente "probablemente", y que no se sabía si el WR cambiaría. Tras el re-fetch (07-sep) la decisión de adoptar el parquet corregido se justificó por el WR (50.20% vs 49.97%, gap long/short 5.62pp→0.54pp), NO por un pico de volumen verificado.
- Verificación con la ventana corregida (este análisis): el volumen medio por barra pica a las 8:30–9:30 CT (2,267/2,105), no en 7:00–8:30 (1,215–1,580); la premisa (c) resultó falsa. Aun así:
- Escaneo de hora de entrada (MGC 364/364, nc=6, flatten 14:30 fijo, motor dyn-nc empírico, p50 cadena 1 año): 7:13 $15,744 (FLAT 51%); 7:43 $18,558 (54%); 8:13 $12,944 (57%); 8:43 $13,185 (62%); 9:13 $13,825 (70%); 9:43 $7,681 (75%); 10:13 $4,768 (79%). Entrar más tarde, hacia el pico de volumen, NO mejora: sube el FLATTEN share (menos tiempo hasta 14:30) y baja el payout. Las diferencias 7:13 vs 7:43 (+18%) están dentro del ruido bootstrap (IC ~$10–26k). Conclusión: la hora 7:13 no queda refutada por el pico de volumen; no hay evidencia para cambiarla. Ninguna acción sobre producción.

### Grid extendido de ancho de SL en MGC + due diligence del pico (19-sep-2026)

**Motor:** EMPÍRICO en todo (`dd_wr/mgc_width_grid.py`: bar-walk real de sesión con FLATTEN + bootstrap del PnL real, XFA con Scaling Plan real vía `xfa_dyn_emp`, cadena `run_cashflow_simulation`). NO se usó el motor binario. Entrada 7:13 CT sin cambios, flatten 14:30, alternar, SL$≈$2,184 (nc=round(2184/SL_ticks)), 12 anchos c×R (R=477 ticks) × 4 RR + baseline.
**RR=1.0, p50 cadena 1 año (FLATTEN share / barras ambiguas):** c=.763(364) $16.2k (51%/0); .50 $29.6k (25%/0); .40 $38.6k (16%/.4%); .35 $41.7k (11%/0); **.30 (143/143, nc=15) $50.7k (7%/.2%)**; .25 $42.5k (4%/0); .20 $29.5k (1.6%/.8%); .175 $27.5k; .15 $29.8k; .125 $27.1k (.2%/1.7%); .10 $14.1k (0/3.5%); .075 $13.3k (0/5%); .05 $1.7k (0/10%). RR<1 siempre peor que RR=1 a anchos ≤0.5R. Pico interior (no en el borde).
**Hipótesis FLATTEN→payout: SOLO PARCIALMENTE respaldada.** De .763R a .30R el payout sube mientras FLATTEN cae 51%→7% (consistente). Pero por debajo de .25R el FLATTEN ya es ~0 y el payout CAE ($42k→$27–30k→$1.7k): el patrón se rompe. El payout sigue mejor al EV por trade (.5R $31, .4R $92, .3R $162, .25R $101, .2R $28, ≤.1R negativo) que al FLATTEN share. A anchos ≤.2R además crecen las barras ambiguas (TP y SL en la misma barra de 5 min, resueltas SL-primero) y el bracket optimista se separa fuertemente (p.ej. .125R $27k vs $34k, .05R $1.7k vs $16k): la resolución de 5 min no basta para anchos ≤.15R; esos resultados no son confiables.
**Due diligence del candidato 143/143 nc=15 vs baseline 364/364 nc=6 (`dd_wr/mgc_dd.py`):**
- Fricción (p50): slip 0/1/2/3/4 ticks: $50.6k/$42.5k/$40.5k/$30.0k/$28.7k (baseline $15.7k/$15.2k/$14.4k/$13.6k/$12.9k); comisión +50%: $47.5k. Sigue >2x el baseline con 4 ticks, pero cae ~40% entre 0 y 4 ticks.
- Kelly: EV/trade +$162 (sd $2,083, t=1.77, p=0.077); baseline −$54 (t=−0.75). nc Kelly (W=$150k) 84, IC90 [6,170] vs nc usado 15 → no está sobredimensionado. El EV positivo es marginal, no significativo al 5%.
- Dependencia: runs test p=0.60, autocorrelación lag1-5 dentro de la banda ±0.086 → sin dependencia detectable. **ALERTA:** EV long −$208 (n=257) vs short +$532 (n=258); signo consistente en los 3 tercios (short +403/+623/+570, long −216/−58/−351) y en anchos .25–.40R. Con "alternar" la mitad long paga la mitad short: el EV positivo viene ÍNTEGRO de los shorts. Sesgo direccional en muestra — contradice la premisa "sin edge direccional" de Camino B; no se explota (sería ajustar dirección a la muestra).
- Sub-períodos: candidato > baseline en H1/H2 y en los 3 tercios (p50 H1 $43k vs $8k, H2 $58k vs $23k; T1 $40k/$10k, T2 $71k/$24k, T3 $44k/$14k); EV del candidato positivo en los 5.
- Bootstrap pareado de días (estadístico S = payout esperado/año, Spearman 0.997 vs p50 cadena en 49 configs): S cand $53.8k vs base $18.0k (x2.99); dif IC90 [$17.1k, $61.3k], P(dif>0)=1.000 (iid y bloques de 10).
- Data snooping: Reality Check de White sobre 74 configs MGC distintas (grid + barrido de productos + baseline), B=1000: p=0.013 (iid), p=0.017 (bloques 10), percentil 95 de V* $28.4k/$27.3k vs V_obs $35.8k. Sobrevive a la corrección de multiple-testing sobre esta familia (no cuenta otros análisis previos de la sesión).
**Veredicto provisional:** robusto a los tests aplicados, PERO no es un candidato listo: (1) EV positivo marginal y de origen direccional (shorts), (2) pico puntual (.30R $50.7k) sobre meseta .25–.40R ($38–51k) — la meseta es la estimación honesta, (3) sample de una sola historia de 515 días, régimen de gold, (4) fricción modelada solo como slippage fijo. Requiere ventana propia de paper trading antes de reemplazar nada. Producción sin cambios.

### Prueba de replica cross-producto del sesgo direccional del candidato MGC 143/143 (21-sep-2026)

`dd_wr/cross_product_direction.py`, motor empírico (bar-walk real). Control MGC (entrada 7:13) reproduce: LONG −$208 / SHORT +$532, Welch p<0.001, short>long en 3/3 tercios, y en c=.25–.40R. Versión A (mismos ticks 143/143 nc=15) y B (mismo ancho relativo .25–.40R, nc por SL$≈$2,184), entrada 8:43 CT, flatten 14:30.
- **M2K:** A: EV −$28 (t −0.65), LONG −$40 / SHORT −$16, dif +24, p=0.79, short>long en 0/3 tercios. B (.25–.40R): EV +12/−65/−59/−29; dif S−L +141/+190/+10/+73, todas con p>0.30; tercios inconsistentes (T2 invierte). Signo de la diferencia positivo pero sin significancia ni consistencia.
- **M6E:** A no es informativo (143 ticks = 3.7R, 515/517 trades FLATTEN). B: EV −$54…−$100 (t hasta −2.4), dif S−L +52/+58/+11/+58, todos p>0.34; short>long en 0/3 tercios en todos los anchos.
- **RESULTADO: el patrón NO se replica** en M2K ni M6E → el sesgo short/long de MGC es específico del régimen/producto/hora de MGC (o suerte histórica), no un artefacto estructural de geometrías estrechas. Salvedades: mismo calendario (2024–2026, no un período independiente), M2K/M6E entran a las 8:43 CT (microestructura distinta a 7:13 en oro) y sus ventanas de datos empiezan en 8:30 CT.
- **Descomposición del payout del candidato** (chain p50, motor empírico, se resta una constante a cada trade): observado (EV +$162) $50.6k; con EV=0 $24.6k; con EV=−$54 (como baseline) $21.8k; baseline 364/364 $15.7k. Es decir ~$6–9k de mejora es geométrica (menos FLATTEN, RR=1, ancho angosto) y ~$26–29k depende del EV de muestra positivo de MGC, que no se replica.
- **DECISIÓN según el criterio del usuario:** 143/143 NO se considera sustituto de MGC-150K. Hallazgo interesante, no accionable. Cerebro 2 continúa con 364/364 hasta el deadline del 1-oct-2026. Producción sin cambios.

### CIERRE: rama research/mgc-wr-improvement PAUSADA hasta el 1-oct-2026 (21-sep-2026)

**Decisión del usuario:** 143/143 (nc=15) NO sustituye a MGC-150K; hallazgo interesante, no accionable. Cerebro 2 sigue con 364/364, sin cambios. Rama pausada completa, sin merge ni push. Única tarea activa relacionada a Cerebro 2 hasta el 1-oct: seguimiento de la mezcla real de producción (`dd_wr/audit_mgc_live_mix.py`, lo corre el usuario con cada ciclo nuevo).

**Pista anotada para el futuro (no candidato):** el efecto geométrico (~$6–9k de mejora en p50: menos FLATTEN, RR=1, ancho angosto) parece real e independiente del sesgo direccional, PERO su estimación fue una aproximación gruesa (restar una constante a cada trade, incluidos los FLATTEN), no una descomposición rigurosa. Si se retoma: diseñar el experimento para aislarlo desde el principio (forzar EV=0 por construcción en la simulación, p.ej. anular la asimetría long/short o simetrizar el PnL por trade, en vez de restar después).

**Resumen de lo aprendido en la rama:** (1) el $31,257 publicado usaba un motor binario que ignora FLATTEN; con la distribución real el p50 es ≈$16k (IC ~$10–26k); (2) el ancho del SL importa más que el RR y que el producto; (3) el pico de MGC a 0.30R sobrevivió fricción, sub-períodos y Reality Check (p≈0.015 sobre 74 configs) pero su EV positivo viene de un sesgo short/long específico de MGC que no se replica en M2K/M6E; (4) otros productos (M2K, MCL, M6E, ZN, ZC) no superan a MGC; (5) origen de 7:13 CT: ENTRY_WAIT=13 es buffer de delay de Massive, la apertura 7:00 se basó en una extrapolación de volumen que la ventana corregida no confirmó, pero entrar más tarde empeora el resultado.

---

## research/cerebro2-cashflow-monthly — flujo de caja mensual real, 50K/100K/150K, 5 cuentas, mezclas (21-sep-2026)

**Motor nuevo (`dd_cash/engine.py`, diario, empírico):** máquina de estados Combine→XFA→nuevo Combine con timing real de cada fee y payout (mes = 21 días de trading, horizonte 12 meses = 252 días), PnL por contrato del bar-walk real (FLATTEN incluido), reglas copiadas de `simulation/monte_carlo.py` y `core/funded_account.py`. Validación: Combine G2 50K primer intento pase 79.2% vs simulador oficial 79.6% (y 80.0% empírico en vivo); MGC 150K 40.3% vs 40.3%; payout XFA por episodio $1,638 vs $1,653 (`xfa_dyn_emp`), <1%. Fix balance>0 en payout (variante corregida; as-is ≈ −2% en p50). Datos alineados por fecha (507 días MES/MGC).
**Pipelines:** A = MGC 364/364 en Combine y XFA (nc 3/4/6 para 50K/100K/150K); B = híbrido Cerebro 1→2: Combine con G2 (MES 100/40, nc 40/80/120 escalado ∝ profit target) y XFA con MGC. OJO: G2 solo está validado en vivo a 50K; el escalado a 100K/150K es supuesto.
**CORRECCIÓN IMPORTANTE de números previos:** `run_cashflow_simulation` (publicado $31,257 y las tablas "a 1 año" de esta sesión) suma DÍAS DE TRADING de los pools contra un horizonte de 365 → su "1 año" son ~365 días de trading (~1.45 años). Por 252 días reales el número comparable baja ~31%: p.ej. MGC 150K pipeline A media $11.7k/año (vs ~$16k "a 1 año" antes).
**Resultados (1 cuenta, 12 meses, payout p10/p50/p90):** 50K-A 2.5k/6.4k/12.9k; 50K-B 5.9k/11.0k/18.1k; 100K-A 2.1k/6.7k/14.9k; 100K-B 8.6k/15.5k/25.2k; 150K-A 3.1k/10.1k/22.3k; 150K-B 13.1k/23.4k/37.5k. Fees/año por cuenta A: $2.7k/$3.0k/$3.9k; B: $3.6k/$4.3k/$5.2k. Mes estable (4–12), P(payout=$0): A 60–65%, B 32–39%; P(neto mensual<0, con OPS $45/mes): A 62–67%, B 38–44%. 5 cuentas (correlacionado = 5×; independiente subestima el colchón p90 3.0–3.4×): 5×150K-B payout p10/p50/p90 $65k/$117k/$187k, colchón p90 $8.5k; 5×150K-A $16k/$51k/$111k, colchón p90 $14.2k. Mezclas de tamaño: correlación de payout anual entre tamaños 0.85–0.99, CV idéntico (B 0.39–0.41, A 0.59–0.68), media aditiva exacta → misma apuesta escalada, sin diversificación.
**Sensibilidad de régimen (1er tercio baja vol / último tercio):** 150K-B media $18.9k / $23.8k; 150K-A $7.6k / $11.3k; P(payout=0 en mes estable) B sube a 27–41%, A a 66–70%. **Costos:** Massive $29–58 (usado $43.5), Pi ~$1.5 (luz ~$0.5 + hardware amortizado), OPS $45/mes; Railway (~$5+ mientras no migre al Pi) y posible cuota de API de ProjectX/TopstepX NO verificados ni incluidos. Sin cambios a producción.

### ACLARACIÓN de encuadre (21-sep-2026): Pipeline B es el plan real, A es el escenario conservador

Evidencia de código: `geometry_scheduler.py` (MES/G2) y `geometry_mgc_scheduler.py` (MGC/XFA) NO se importan entre sí ni nada los importa a ambos; cada uno usa su propio archivo en el gist compartido (`geometry_mes_log.json` / `geometry_mgc_log.json` + `geometry_mgc_pending.json`) y solo comparten el GIST_ID/token y utilidades (`execution/gist_store.py`, `strategies/geometry_pure.py`, `contracts.py`, `telegram_bot.py`). Ninguno lee el log del otro. Ninguno de los dos contiene código de colocación de órdenes (solo señal/Telegram/log; DRY_RUN etiqueta paper vs live). La transición Combine(G2)→XFA(MGC) NO existe en código: es manual (SOP/portal SHEDGE, fuera de este repo, no verificable desde código). El scheduler MGC declara en su docstring que NO simula la máquina de estados Combine+XFA. Pipeline B (`dd_cash`) es la SIMULACIÓN del plan real, no algo automatizado. Se presentó A primero por herencia del motor publicado (Combine y XFA con MGC), error de encuadre corregido: B = referencia principal, A = escenario conservador (nunca se hace la transición).

### SOP confirmado y colchón inicial recomendado, 1 cuenta 50K-B (21-sep-2026)

**Plan inicial confirmado por el usuario:** 50K-B (G2 pasa el Combine → transición manual → MGC 364/364 en XFA). 100K/150K = aspiración futura, NO plan inicial, hasta validar G2 escalado en producción real.
**Colchón (dd_cash/cushion_50kb.py, cushion_stress.py; motor diario empírico, 40,000 trayectorias, incluye operación $45/mes, lag de transición manual 2d y cobro de payout 5d):** el colchón es el máximo de la caja acumulada negativa (fees + operación − payouts cobrados). 6 meses, p95/p99: base $1,822/$2,299; baja vol $1,476/$1,863; régimen reciente T3 $2,497/$2,744 (sin lags T3 p99 $3,189). P(caja cae bajo −C, 6m, base|T1|T3): $1,500 10%|5%|24%; $2,000 3%|0.5%|12.5%; $2,500 0.2%|0|4.4%; $3,000 0|0|0. Estrés con payouts ×0 (ninguno cobrado en 6 meses, reiniciando Combines): p99 ≈ $2,495 (base) / $2,842 (T3). **RECOMENDADO: $3,000** (mínimo aceptable $2,000, riesgo 3–12% de agotarlo en 6m; cómodo $3,000–3,500).
**Patrón temporal:** fees concentrados al inicio (mes 1 $347 vs $300 estable); valle mediano −$392 en el día 20; caja mediana ≥0 desde el día 28 (media día 14); primer payout mediano día 23 (p10 11, p90 59); P(sin payout en 3m) 8%, 6m 1%. Caja acumulada P(<0): fin mes 1 59%, 2 38%, 3 30%, 6 15%, 12 5%. Rachas de meses con payout=0 (meses 1–6): P(≥2) 52%, P(≥3) 18%, P(≥4) 6%. Después del primer payout los eventos son sin memoria: no hay un calendario que planificar. Palanca real = evaluar por caja ACUMULADA (no por mes) y no retirar hasta que la caja supere el colchón.

### P&L tradicional 50K-B, mensual/trimestral/anual (21-sep-2026)

**Reformateo, no recálculo:** misma simulación de `dd_cash/engine.py`+`data.py` ya validada (Account 50K, XFA_50K, nc_c=40/nc_x=3, MES/G2 en Combine, MGC en XFA, sin lag de transición, seed=2026, T=20000). Único cambio de código: `fee_m` se separó en `fee_c_m` (Combine: mensualidad + reinicio tras quiebre) y `fee_a_m` (activación). Verificado `fee_c_m+fee_a_m==fee_m` exacto y payout p10/p50/p90 reproduce idéntico a lo ya reportado (5,921/10,986/18,140) → no es una corrida nueva, es la misma con más detalle.
**Combine fee real (core/prop_firm.py, TOPSTEP_50K):** monthly_fee=$49, activation_fee=$149 — el "$99-149" que el usuario propuso para la inversión inicial NO coincide con el código; corregido a $49 (primera mensualidad, antes de pasar).
**HALLAZGO IMPORTANTE — sum-of-medians ≠ median-of-sum, brecha grande:** cumsum de la fila "neto mes" (mediana por mes, independiente): $2,464 a 12 meses. Percentil REAL de la trayectoria acumulada (mediana de la distribución conjunta): $6,791 a 12 meses — 2.75x más alto. Causa: ~39% de los meses en régimen estable tienen payout=$0 (mediana mensual $250-270), pero cuando SÍ hay payout la distribución es fuertemente asimétrica a la derecha (media mensual ~$655, muy por encima de la mediana). La suma de 12 meses se acerca más a la media (suma de medias=$7,407, coincide exactamente con la media de la acumulada) que a la suma de medianas. Se usó el percentil REAL de la acumulada para "Utilidad Acumulada" (no cumsum de la fila mensual) y se documentó la brecha explícitamente, no se ocultó.
**Corrección a la premisa del usuario:** "día 28 ≈ mes 1" es incorrecto con la convención mes=21 días de trading (mes1=días1-21, mes2=días22-42) — el cruce a mediana acumulada ≥0 ocurre en el día 27, que cae en el MES 2, no en el mes 1 (fin de mes 1: mediana −$370).
**Pendiente estable:** mediana de neto mensual ~$250-270 desde el mes 2 en adelante (no hay "arranque" adicional más allá del mes 1); media mensual ~$655-660 desde el mes 2 (la brecha mediana/media es el mismo efecto de asimetría de arriba — reportado ambas, no solo una).

## URGENTE (24-sep-2026): "Consistency Target 55%" verificado + hallazgo mayor sobre el 81.4% de G2

**Fuente oficial (help.topstep.com, leído directo):** Consistency at Topstep (8284208): Combine "Consistency Target 55%" — la mejor día debe ser <= 55% del profit total; si se excede **NO es falla: el Profit Target SUBE a best_day/0.55** (ejemplo oficial 50K: best day $1,800 -> target $3,273). Es una regla REAL y ACTIVA del Combine (el 40% es del XFA, aparte). Trading Combine Parameters (8284197): "puedes pasar en 2 días". MLL (8284204): monitoreado EN TIEMPO REAL incluyendo P&L no realizado; tocarlo liquida la cuenta inmediatamente. DLL (10490293, publicado 30-jun-2026): OPCIONAL en el Combine (se agrega al comprar), $1,000 en 50K, liquida y bloquea la sesión, no es violación.
**Consistencia NO es el problema de G2** (`scripts/g2_consistency_check.py`): un TP neto de $1,951 sobre el profit total de 2 TP ($3,902) = 50% <= 55%, pasa en 2 días (target oficial subido a $3,548 < $3,902). El simulador del repo usa cap 50% ESTRICTO (2 TP iguales fallan) y solo días positivos — más severo que la regla real: pass 81.5% (repo) -> 83.0% con cap 55%; semántica oficial (neto) 81.85%. La preocupación "TP de $2,000 = 66% del target" compara contra el target, no contra el profit total al pasar.
**HALLAZGO MAYOR (encontrado al verificar):** el 81.4% teórico depende de que el simulador CLIPEE toda pérdida diaria a -$1,000 (DLL, `monte_carlo.py`) sin cambiar el WR (calibrado con SL a 100 ticks = -$5,049). Con las reglas reales: sin DLL (default), una pérdida de -$5,049 rompe el MLL (>-$2,000): pass 49% a nivel día; y como el MLL se monitorea en tiempo real con P&L no realizado, el SL de 100 ticks NUNCA se alcanza — la liquidación ocurre a -40 ticks (-$2,000): bar-walk empírico WR 50.6%, pass **26%**; con DLL agregado (liquida a -20 ticks) WR 34.4%, pass **25%**. Además la contabilidad del propio paper (SL = -$5,049 -> QUIEBRE al primer SL, PASE al llegar a +$3,000, sin chequeo de consistencia ni liquidación intradía) implica pass ~48%, NO 81%: el 80.0% empírico "vs 81.4% teórico" no se compara manzana con manzana. Sin Gist en esta sesión: `scripts/audit_g2_pass_reality.py` (solo lectura, lo corre el usuario) para ver los intentos reales. NO verificado: si un pago debe esperar (decisión del usuario).

### 24-sep-2026 (cont.): ¿el scheduler de producción modela la liquidación MLL? NO. Re-simulación con las reglas oficiales completas.

**geometry_scheduler.py (main) — evidencia de código:** el loop de monitoreo calcula `unreal` solo para el log (`log.info`); las únicas salidas son `price <= sl_price` / `price >= tp_price` (100/40 ticks) o el flatten de las 14:30 CT. No compara el P&L no realizado contra el piso MLL ni contra el DLL, y solo usa el último cierre del poll de 60s (no high/low intrabarra). El QUIEBRE se evalúa DESPUÉS de cerrar el trade con P&L realizado (`_check_attempt_reset`). Por tanto el paper trading comparte el defecto del simulador (SL de 100 ticks se ejecuta completo, -$5,049) — con un matiz: la contabilidad del paper (quiebre al primer SL) NO usa el clip de DLL del simulador, así que su pass implícito es ~48%, no 81%. El paper además sobrecuenta TP: en el bar-walk, ~24% de los "TP" de paper (336 vs 254) habrían sido liquidados antes por el MLL real.
**Motor nuevo `scripts/g2_real_rules_scan.py`:** bar-walk 5min MES (515 días, 1 trade/día alternando, 9:45–14:30 CT), liquidación en tiempo real por distancia al piso trailing (o DLL $1,000 opcional), consistencia oficial (target = max($3,000, mejor_día/0.55), mejor día no se resetea con pérdidas, mínimo 2 días).
**G2 (nc=40, TP40/SL100):** paper-equivalente (sin consistencia ni liquidación) pass 48.4%, 2.14 días, 66.7 pases/año; +consistencia 55%: 47.8%, 2.17 días (**-0.6pp, +0.03 días: la consistencia casi no afecta a G2**, porque 2 TP de $1,951 = 50% del profit total y el target ajustado $3,548 < $3,902); +liquidación MLL real (DLL off): **25.5%**, 2.06 días, 41.7 pases/año; con DLL on: 24.9%, 3.66 días, 19.9 pases/año. Aislada, la liquidación explica todo el deterioro (consistencia OFF: 25.7%).
**Barrido Camino B bajo reglas reales (1,080 configs, `g2_real_rules_grid.py`):** mejor pass ≈ 28–31% (nc 25, TP 50, SL 40: 31.2%; H1/H2 consistentes), DLL on ≈ 27–28%; ningún nc bajo ni TP chico lo rescata (nc 5–10 necesitan 8–26 días y no superan 26%). Cota teórica de juego justo (MLL trailing $2,000 que se traba en 0, target $3,000): exp(-1)·2/3 ≈ 24.5%. **El pass de ~80% de Camino B dependía de que las pérdidas fueran menores que el movimiento de precio (clip de DLL en el simulador), algo que las reglas reales no permiten: con liquidación en tiempo real el juego pasa a ser ~simétrico y el pass rate estructural es ~25–30%.**
**Retroactivo sobre los intentos reales:** requiere el Gist (no disponible en esta sesión); `scripts/audit_g2_pass_reality.py` extendido (solo lectura): por intento imprime mejor día, target ajustado oficial, día de pase oficial vs. del scheduler, y si alguna pérdida realizada excedía la distancia al piso (liquidación real). Probado con un log simulado; no corrido contra datos reales.
Límites: barras de 5min (SL-primero en barras ambiguas), sin slippage de liquidación, sin candados intradía del DLL sobre P&L no realizado más allá de la distancia, 515 días de una sola historia.

## PARTE A — Auditoría exhaustiva de help.topstep.com (24-sep-2026)

Método: sitemap oficial (`help.topstep.com/sitemap.xml`, 53 artículos) leído artículo por artículo (las 4 colecciones reportan 56 en total: 3 no identificados; no se leyeron los Terms of Use, documento legal aparte). **Confirmado contra fuente oficial:** MLL 50K $2,000 y monitoreo en tiempo real con P&L no realizado (liquidación inmediata; XFA cerrada permanentemente); DLL opcional ($1,000 en 50K, liquida y bloquea la sesión); micros 50/100/150 (10:1); comisiones MES/MNQ/M2K $1.22, MGC $1.92, MCL $1.52, M6E $1.00, ZN $2.62, ZC $5.28; pago 90/10; 5 días de $150+ y "net profit > $0 since last payout"; XFA empieza en $0, hasta 5 XFA, inactividad 30 días, sin mensualidad tras el Combine; flatten 3:10 PM CT; $149 activación (Standard).
**Supuestos del código INCORRECTOS o sin fuente:** (1) `TOPSTEP_150K.monthly_fee=149` → oficial Standard $199 (100K $99 y 50K $49 ok; ruta "No Activation Fee": $95/$149/$229); (2) consistency_cap 50% estricto y solo días positivos → oficial 55%, sube el target a mejor_día/0.55, profit neto; (3) tope de payout XFA $5,000 para todos los tamaños → Standard: 50K $2,000, 100K $3,000, 150K $5,000 (Consistency path 3k/4k/6k); (4) DLL como clip obligatorio → opcional y liquida; (5) sin liquidación MLL en tiempo real (ver secciones anteriores); (6) `XFASpec.floor_lock_level = +mll_distance` vs oficial "el MLL se traba al llegar al balance inicial" (en XFA = $0) — por verificar; (7) tiers del Scaling Plan: la tabla oficial es una imagen, sin fuente textual verificada; (8) MLL/profit target de 100K/150K: las páginas leídas solo dan cifras del 50K ($2,000; target $3,000 vía el ejemplo de consistencia). Reglas de cadencia de fees: rebill cada 30 días naturales, fallar NO cancela la suscripción (sigue cobrando), reset = mismo precio que la mensualidad, 1 Reset Credit por rebill (un reset empuja el rebill 30 días), 2 resets/cuenta/día, pasar cierra la cuenta y cancela la suscripción.
**Reglas nuevas relevantes al proyecto:** API `TopstepX API Access` $14.50/mes con código (regular $29); requiere cuenta ProjectX; permitido en cuentas Simulated/Practice/Evaluation, **NO en Live Funded**; ejecución solo desde dispositivo personal (VPS/VPN/servidores remotos prohibidos para transmitir órdenes; servidor puede observar/registrar) — consistente con el diseño Railway/Pi; HFT prohibido; órdenes API finales. Practice Account gratis con Combine activo ($150K sim, 15 lotes, requiere IDV). LFA: sin automatización ("API Gateway solo simulación"), call-down sin aviso por actividad que "parece apuesta"; Pro Account para jurisdicciones sin live; W-8BEN para no-US. Cierres anticipados de feriados: 26-nov cerrar 11:45 CT, 27-nov 12:00, 24-dic 12:00 (posiciones se liquidan) — los schedulers flatten fijo 14:30 → hueco. Debe operarse el front month (mayor volumen; TopstepX auto-rota). Límite de zona 2% de precio-límite; restricciones ~10 min alrededor de CPI; límites de posición se endurecen en alta volatilidad; stops = órdenes de mercado (slippage puede exceder el MLL). Trade Copier oficial solo Combine/XFA. IDV antes de la 2a compra; un perfil por trader; tarjeta propia; chargebacks prohibidos; garantía 14 días (primera compra).
**RIESGO DE CONDUCTA (no es un veredicto):** conducta prohibida incluye "excessive purchases of Combines or Resets", "account stacking — repeatedly hitting the MLL in one account and switching to another to repeat high-risk attempts", "trading inconsistent with real futures markets", "any other conduct ... that games the market, is not viable, or is not responsible trading", y entre los ejemplos de explotación de SIM "tight brackets". El Responsible Trading Program se dispara por pérdidas mayores que ganancias, max position en la mayoría de trades y varias cuentas tocando MLL el mismo día (consecuencias: DLL automático en Combines nuevas, XFA solo Consistency 40%). El patrón de G2 (pérdida 2.6x la ganancia, ~80% del máximo de micros, varios Combines seguidos que se pierden) y las 5 cuentas idénticas tocan varios de estos criterios; Topstep tiene discreción.
**MLL en XFA:** aplica igual (tiempo real). MGC_XFA_150K: SL ≈ $2,184+comisión vs distancia 4,500 al inicio → dentro de rango, pero tras un payout el piso vuelve a $0 y el balance baja → la distancia al piso puede ser menor que el SL y la liquidación ocurriría antes. No cuantificado.
**P&L 50K-B corregido** (`dd_cash/engine_real.py`, `real_pnl.py`; Combine con reglas reales, XFA cap $2,000): 1er Combine pase 26.3%, 2.06 días; payout anual p10/p50/p90 $4,959/$9,667/$16,206; fees medias $5,349; neto acumulado 12m p10/p50/p90 −$1,869/$3,532/$11,001; P(año<0) 22%; mediana de caja acumulada cruza a >=0 en el día 49 (mes 3, antes día 27/mes 2); colchón 6m p95/p99 $3,125/$3,856 (sin lag) y $3,009/$3,560 (lag 2d/5d) vs $1,822/$2,299 anterior; media neto/mes estable ≈ $386 (mediana ≈ −$40..−$80, muy asimétrica). Geometría alternativa nc25/TP50/SL40 (pass 31%) NO mejora el neto (media $332/mes, colchón similar).

## Correcciones de código, MLL en XFA y PARTE B (24-sep-2026)

**Código corregido (sin tocar producción; rama de investigación):** `core/prop_firm.py` `TOPSTEP_150K.monthly_fee` 149 → 199; `core/funded_account.py` tope de payout XFA Standard por tamaño (50K $2,000 / 100K $3,000 / 150K $5,000, antes $5,000 fijo). 205 tests pasan. (150K sin cambio de cap; producción usa solo profit_target/mll/min_winning_day/payout_pct.)
**RTP — cifras oficiales:** el artículo (13620045) NO da umbrales numéricos ("majority of trades", "losers smaller than winners"); revisión discrecional del Risk Team "monitoreada en el tiempo", normalmente hay advertencia previa pero pueden mandar directo. No es posible medir la distancia a un umbral; se usan PROXIES medidos: nc/máximo, SL$/TP$, toques de MLL por 100 días.
**MLL en tiempo real dentro de la XFA (`scripts/xfa_liquidation_check.py`, MGC 364/364 sobre tablas de barras reales, lifetime de una XFA):** 150K nc6: payout medio $1,595 → $1,158 (lock del código +MLL) / $1,346 (lock oficial en $0), P(>=1 payout) 46% → 37/38%, vida 11.5 → 9.2/9.8 días, 20% de los trades con liquidación vinculante (10% liquidados antes del SL). 100K nc4: $1,055 → $761/$898. 50K nc3: $671 → $484/$620 (P>=1 payout 37.5% → 29/32%). La distancia al piso tras un payout (piso vuelve a $0, balance baja 50%) es la causa. El lock del piso (+MLL del código vs $0 oficial) mueve ~16%: verificar.
**PARTE B (`scripts/partb_grid.py`, `partb2.py`; Combine 50K, reglas reales, DLL off):** límite duro SL$ <= 40% del MLL ($800) y nc <= 50% del máximo → 767 configs en 7 productos x 3 horas: ninguna geometría sin edge supera ~28% de pass (MES 8:43 nc16 TP75/SL25 RR3: 27.5%, costo por pase $328, PERO 26 días por pase); el top absoluto (MGC 7:13, RR3, 42.9%, costo $265) es el bolsillo de EV de muestra de MGC que no replicó en M2K/M6E — no se usa. Salida parcial + breakeven (mitad en TP1, resto a TP2/BE): no mejora (costo por pase $331–$776 vs $328 sin parcial). DLL on: sin efecto material. Fricción: RR3 pierde (slip 2 ticks: pass 21%, costo $382); RR1 no. Costo por pase = fees por intento (incl. renovación cada 21 días)/p + $149. Selección H1→H2: top5 por H1 (costo 271) → 350 en H2 (mediana del grid 502): la selección infla ~30% pero sigue mejor que la mediana.
**Frontera velocidad vs conducta (MES 8:43 CT, TP=SL en ticks, nc):** G2 actual (9:45, nc40, TP40/SL100): pass 25.8%, 6.0 días/pase, costo $339, 48 toques de MLL/100d, 80% del máximo, pérdida:ganancia nominal 2.5:1, intentos p50/p90/p99 3/8/16, fees+act $296/$541/$933, días 5/12/23. **A: nc16 TP=SL=100:** pass 28.9%, 7.1 días/pase, costo $319, 34.7/100d, 32% del máximo, 1:1, intentos 3/7/14, $296/$492/$835, días 5/14/27; H1 pass 29.9% / H2 27.8% (G2: 28.9%/23.2%); fricción slip 3: $323; DLL on: $320. **B: nc12:** 31.1%, 11.8 días/pase, $307, 18.7/100d. **C: nc8:** 34.5%, 17.9 días/pase, $291, 10.6/100d, intentos 2/6/11, $247/$443/$688, días 14/36/68. Ingreso mensual aprox. por ciclo (XFA 50K payout $620, vida 9.2d): G2 ≈ $388, A ≈ $388, B ≈ $313, C ≈ $255. Conclusión: A iguala el ingreso de G2 con menor costo por pase y ~la mitad de los proxies de conducta; ninguna geometría reduce el costo por pase de forma material (-6% a -14%). Sin corrección formal de multiple-testing (decenas de configs); una sola historia de 515 días; barras de 5min.

### PRIORIDAD 1 (25-sep-2026): piso del MLL en XFA — RESUELTO con texto oficial

`help.topstep.com/8284204` (What is the MLL): "Your XFA balance starts at $0 ... For a 50K XFA, your MLL starts at -$2,000 and trails upward. Once your balance reaches $2,000, the MLL locks at $0 permanently." "After your first Payout: Your MLL is set to $0 regardless of where it was before. The remaining balance becomes your effective loss floor. Balance $10,000. Take a $5,000 Payout. Balance $5,000, MLL $0." Ejemplo numérico oficial: día 1 +$500 → balance $500, MLL -$1,500; día 2 +$1,500 → balance $2,000, MLL trabado en $0. `8284233` (Payout Policy): "Your MLL is set to $0 after your first Payout. If it's already at $0, it stays there. The remaining balance becomes your effective loss floor." / "resets to $0 permanently". `8284215` (XFA Parameters): no agrega detalle (remite al artículo de MLL; dice que la cuenta se liquida y cierra al final del día tras el breach).
**Lectura oficial = piso trabado en $0** (no en +MLL). El código (`XFASpec.floor_lock_level = +mll_distance`) era incorrecto; corregido a 0.0 (`core/funded_account.py`), 2 tests actualizados (la distinción política "every_payout"/"first_payout_only" queda moot: con el piso permanente en $0 ambas coinciden). Incertidumbre residual honesta: los textos dan cifras solo del 50K; 100K/150K se asumen análogos (lock en $0 al llegar el balance a su mll_distance). NO queda ambigüedad entre dos lecturas: el rango $1,158–$1,346 de anoche se reduce a **$1,346** (150K con liquidación en tiempo real; lock oficial). Números XFA MGC 364/364 con el lock oficial: 150K sin liquidación $1,772 → con liquidación **$1,346** (-24%; P(>=1 payout) 47.1% → 38.1%; vida 12.2 → 9.8 días); 100K $1,181 → $898; 50K $822 → **$620** (P(>=1) 39.6% → 31.6%).


## 25-sep-2026 — Terms of Use de Topstep (topstep.com/terms-of-use, "Last Updated" 21-sep-2026)

Lectura via WebFetch (extraccion por modelo auxiliar, NO texto literal completo: las citas son las que devolvio la extraccion; verificar literalmente antes de decisiones legales).
- Sec. 27 (conducta prohibida): aprovechar el entorno simulado (slippage, queue positioning, gaps, alto volumen); "software, artificial intelligence, ultra-high speed, or mass Data entry" que pueda dar ventaja injusta; operar fuera del best bid/offer; feed externo/lento; posicion a <2% del lock limit. Consecuencia: eliminar profits, borrar dia, reset o ban; si se lucro aprovechando el simulador, se pierde derecho a retiros de OTRAS cuentas. Todo "a sole discretion". No hay umbrales numericos de conducta (solo 2% lock limit).
- Sec. 28: VPN/VPS prohibidos, terminacion y perdida de profits (consistente con help center; relevante para Pi vs Railway en cuentas Combine/XFA).
- Sec. 8: "market standard risk management": no abrir tamaños sustancialmente mayores/menores o distinto numero de posiciones vs otros trades (G2 nc=40 y Candidato A nc=16 son fijos; MGC nc=6 fijo).
- Sec. 16: una sola Account personal; multiples cuentas => suspension/terminacion a discrecion.
- Sec. 3: sin reembolsos; suspension de ordenes si "unusually large number of orders in unreasonably short period".
