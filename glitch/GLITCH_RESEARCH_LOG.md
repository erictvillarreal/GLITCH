...
## CIERRE: checkout huérfano identificado y renombrado (22-sep-2026)

El directorio por defecto de esta sesión (distinto del checkout real usado toda la sesión, `GLITCH-clean`, sincronizado con `origin/main`) resultó ser un checkout huérfano: `main` local desincronizado 61 commits atrás de `origin/main`, con 7 commits propios nunca pusheados (26-ago-2026, 16:20–18:05, sin git remote configurado según su propio commit `be3160f`). Investigados los 7 uno por uno: **ningún trabajo perdido** — cada pieza sustantiva (fix de ZC, `geometry_scheduler.py`/`geometry_pure.py`/`execution/contracts.py`, split `pass_days`/`blown_days`, criterio de graduación de `DRY_RUN`) tiene un equivalente ya integrado en el linaje real, creado esa misma noche o horas después (commits `99497ec` 18:27 y `27d759e` 23:01) — evidencia de dos checkouts locales trabajando en paralelo esa noche, uno conectado a remoto que se volvió el linaje real, el otro abandonado sin push. Renombrado por seguridad (confirmado por el usuario, no borrado): `GLITCH-main` → `GLITCH-main.orphaned-2026-08-26`. Sin cambios de código en ningún branch. `GLITCH-clean` confirmado como único directorio de trabajo real de esta sesión, sin ambigüedad futura.

## `scripts/verify_orderside_demo.py` — script listo, NO ejecutado (22-sep-2026)

Resuelve el TODO crítico de `OrderSide` (doc oficial vs `brokers/projectx.py` invertidos, ver 11-sep-2026) de forma automática: coloca una orden mínima (`side=0`, `size=1`, MARKET) contra una cuenta de PRÁCTICA, lee `Position/searchOpen` antes y después, y da el veredicto (compra o venta) según el signo de la posición resultante — sin confiar en ninguna de las dos fuentes contradictorias. Confirma que la cuenta está flat antes de operar, pide dos confirmaciones interactivas explícitas, y siempre intenta flatten al final (éxito o error). Loguea cada request/response completo a un archivo local, listo para pegar como evidencia.

**NO probado contra la API real** (este agente no tiene credenciales de ProjectX) — los nombres de campo vienen de la investigación ya documentada y de `brokers/projectx.py`; si la API real devuelve un error de campo/endpoint no reconocido, queda logueado completo para ajustar sin adivinar de nuevo.

**Instrucción explícita del usuario (22-sep-2026): NO ejecutar contra ninguna cuenta (demo o real) hasta la Fase 3 del plan ya confirmado** (Combine pagado, Raspberry Pi validado en Telegram hasta el 15-oct-2026). El script tiene un gate adicional (`GLITCH_PI_PHASE3=si`, verificado: aborta sin ninguna llamada a la API si no está seteado) separado de sus propias confirmaciones interactivas de seguridad — para que no se corra por accidente antes de esa fecha. Trabajo en esta rama (`design/pi-execution`) se detiene aquí hasta que el usuario retome el tema.

## Sección 3 del playbook reescrita como walkthrough real de Terminal de Mac (23-sep-2026)

Urgente: el usuario recibe el Raspberry Pi físico este domingo (28-sep-2026). Reescrita la Sección 3 de `RASPBERRY_PI_MIGRATION_PLAYBOOK.md` de una lista de conceptos generales a comandos exactos, copiables, en orden: instalar Raspberry Pi Imager por Homebrew, generar llave SSH en el Mac ANTES de flashear (para configurar login sin contraseña desde el primer arranque, headless), configuración avanzada del Imager (hostname `glitch-pi.local`, SSH por llave, timezone `America/Chicago` — mismo TZ que Railway, evita el mismo bug de `ct_logging.py`), conexión SSH desde el Mac con fallback si mDNS no resuelve (`arp -a` por prefijo MAC de Raspberry Pi, o revisar el router), setup del sistema y clonado del repo (`git checkout design/pi-execution`), venv con SOLO `requests`, verificación de conectividad saliente (aclarado que corre DENTRO del Pi por SSH, no desde el Mac), endurecimiento de SSH, y el unit file de systemd preparado pero **explícitamente NO arrancado**.

**Split confirmado y verificado de nuevo hoy:** ningún ítem de la Sección 3.1–3.5 depende de los 5 bloqueantes (re-confirmados sin cambios: `pi_executor.py` sigue con 8 `NotImplementedError`; `grep "orden_pendiente"` en los dos schedulers de Railway sigue en 0; `OrderSide` sin verificar; "Auto OCO Brackets" sin confirmar; credenciales de práctica sin gestionar) — todo eso es software/cuenta, no hardware. La Sección 3.6 (systemd) se prepara pero no se habilita, por la misma razón. Tabla de la Sección 6 actualizada para reflejar el hardware llegando y el split domingo/después.
