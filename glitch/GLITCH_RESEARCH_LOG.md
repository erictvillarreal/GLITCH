...
## Tarea 1: contador real de elegibilidad de payout (Cerebro 2, MGC/XFA) (22-sep-2026)
...
## Implementado el hallazgo lateral real: profit-gate desde el segundo payout (22-sep-2026)

Pedido explícito del usuario tras la verificación del "$55k" (sección anterior). `_check_payout_eligibility_crossed()` se extendió reemplazando su lógica interna por `_replay_payout_cycles(paper_log, intento)` (recorrido cronológico único, re-derivado completo del log en cada corrida — mismo principio que `_current_intento()`, ningún campo nuevo separado que se pueda desincronizar; "persistir en el Gist" para este dato significa lo mismo que ya significa para "intento": vive implícito en el log ya persistido).

**Reglas modeladas (verificadas contra help.topstep.com):**
1. El conteo de 5 días ganadores se **reinicia** tras cada payout (no es un múltiplo-5 acumulado sin reinicio).
2. A partir del **segundo** payout: se exige además profit neto ≥ $0.01 desde el balance del payout anterior. El **primer** payout está exento.
3. Si (1) se cumple pero (2) no (a partir del 2º payout): el intento queda **bloqueado** — el conteo NO se reinicia, y el profit-gate se re-evalúa cada día siguiente (incluso un día que en sí mismo no es "ganador" puede desbloquear el payout si mueve el balance por encima del último payout).

`_check_payout_eligibility_crossed()` cambió de signature (antes `(int, int)`, ahora `(list, list)` — compara el número de eventos disparados, no un conteo de días) — dispara solo cuando el número de eventos sube, nunca por seguir bloqueado o por seguir ya elegible. El mensaje `[ELEGIBLE PARA PAYOUT]` y la línea "Next Payout" del SUMMARY usan ahora `_replay_payout_cycles()`; "Next Payout" muestra explícitamente "(BLOQUEADO — profit neto desde el último payout: $X, requiere >= $0.01)" cuando aplica, para que el estado nunca quede oculto.

**Tests:** 7 nuevos en `TestReplayPayoutCycles` (primer payout exento, reinicio real vs módulo-5, segundo payout disparado con profit positivo, segundo payout BLOQUEADO con profit negativo, desbloqueo en un día posterior no-ganador, sin fuga entre intentos), 4 reescritos en `TestCheckPayoutEligibilityCrossed` (nueva signature basada en eventos), 2 actualizados en `TestPayoutEligibilitySimulatedSequence`. 218/218 tests del repo completo pasan.

**No bloqueante para el primer payout que está por ocurrir** (confirmado con un walkthrough manual del escenario real: 4 días ganadores → 5º día dispara correctamente, sin condición de profit; el día siguiente con un SL no reavisa) — el primer payout sigue funcionando exactamente igual que en la implementación original de esta sesión.
