"""
Glitch — Medicion EMPIRICA del delay real de Massive para MES
(07-sep-2026)
========================================================================
Ultimo bloqueo tecnico antes de diseñar el punto de entrada del
scheduler en vivo de MES (Candidato A). Mismo principio de "medir, no asumir"
...
Metodo: resuelve el contrato front-month real de MES (reutiliza
...
ENTRE SEMANA en horario de mercado activo):
    ...
    python scripts/probe_massive_mes_delay.py
"""
...
def _weekday_warning():
    ...
    if weekday == 5 or (weekday == 6 and now_ct.hour < 17) or (weekday == 4 and now_ct.hour >= 16):
        print("!" * 80)
        print("ADVERTENCIA: parece que el mercado de MES esta CERRADO ahora mismo")
...
def main():
    _weekday_warning()
    ticker, last_trade_date = resolve_front_month("MES")
