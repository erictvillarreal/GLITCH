...
def _attempt_trailing_floor(paper_log: list, intento: int) -> float:
    ...
    las cifras de negocio publicadas). Auditoria retrospectiva contra
    el Gist real (16-sep-2026, MGC_XFA) confirmo que la formula de
    abajo reproduce exactamente el mismo floor que el ratchet completo
    de TopstepMonteCarloSimulator recorrido entrada por entrada
    (scripts/audit_mgc_trailing_mll_2026_09_16.py): floor = min(peak +
    MLL_THRESHOLD, 0) -- MLL_THRESHOLD ya viene negativo, peak >= 0
    siempre por construccion de _attempt_peak(), asi que esto reproduce
    floor=MLL_THRESHOLD cuando no hay pico (sin cambio de
    comportamiento en ese caso) y ratchet hacia arriba (tope en 0) a
    medida que el pico crece.
    """
...
def _check_attempt_reset(attempt_pnl_after: float, profit_target: float, mll_threshold: float) -> Optional[str]:
    ...
    de red). mll_threshold ya viene NEGATIVO -- desde el fix del
    16-sep-2026, el llamador pasa el floor TRAILING real
    (_attempt_trailing_floor()), no la constante MLL_THRESHOLD directa
    -- esta funcion en si no sabe ni le importa cual de los dos es,
    sigue siendo una comparacion pura.
    """
...
def run():
    ...
    #         resumen diario normal. CORREGIDO (16-sep-2026): floor
    #         TRAILING real, no MLL_THRESHOLD fijo -- ver
    #         _attempt_trailing_floor() arriba. paper_log ya incluye la
    #         entrada de HOY (append de arriba), asi que el pico de hoy
    #         ya cuenta para el floor de este mismo chequeo. ──
